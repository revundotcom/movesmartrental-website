...
export default function JobFilterList() {
  ...
  return (
    <div>
          ...
          jobsByCountry.map(countryGroup => (
            ...
            >
              ...
              <div className="flex flex-wrap items-baseline gap-2 border-b border-slate-100 pb-4">
                <h3 className="flex items-center gap-2 text-2xl font-bold text-[var(--brand-navy)]">
                  <Globe2
                ...
                {countryGroup.regions.map((regionGroup) => (
                  <div key={regionGroup.region} className="space-y-6">
                    <div className="flex items-center gap-2">
                      <MapPin
                        className="h-5 w-5 text-[var(--brand-emerald)]"
                        aria-hidden="true"
                      />
                      <h4 className="text-lg font-bold text-[var(--brand-navy)]">
                        {regionGroup.region}
                      </h4>
                    </div>

                    {/* Cities */}
                    <div className="space-y-7 pl-7 border-l-2 border-slate-100 ml-2">
                      {regionGroup.cities.map((cityGroup) => (
                        <div key={cityGroup.city}>
                          <h5 className="text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--brand-navy)]/65 mb-3">
                            {cityGroup.city}
                          </h5>
                              ...
                              <li key={role.slug}>
                                ...
                                >
                                  <div className="min-w-0 flex-1">
                                    <h6 className="text-base font-bold text-[var(--brand-emerald)] transition-colors">
                                      {role.title}
                                    </h6>
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="mt-12 flex items-center justify-between border-t border-slate-200 px-4 py-3 sm:px-6">
