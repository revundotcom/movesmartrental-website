...
export async function POST(req: NextRequest) {
  try {
    ...
    const { firstName, lastName, email, phone, linkedin, resumeUrl, whyYou, referral, role, country, state, city } = body;
    ...
    const { error } = await resend.emails.send({
      ...
      to: ["careers@movesmartrentals.com"],
      replyTo: email,
      subject: `New Application: ${role} — ${firstName} ${lastName}`,
      html: `
        <h2>New Job Application — MoveSmart Rentals</h2>
        <table style="border-collapse:collapse;width:100%">
          ...
          <tr><td style="padding:8px;font-weight:bold;border-bottom:1px solid #eee">Residential Location</td><td style="padding:8px;border-bottom:1px solid #eee">${locationStr || "Not provided"}</td></tr>
        ...
        <p style="margin-top:24px;color:#666;font-size:12px">Sent from movesmartrentals.com/careers</p>
      `,
    });
